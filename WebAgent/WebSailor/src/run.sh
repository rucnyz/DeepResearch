...
CUDA_VISIBLE_DEVICES=0,1,2,3 python -m sglang.launch_server \
    --model-path $MODEL_PATH --host 0.0.0.0 --tp 2 --port 6001 --attention-backend flashinfer &

ORIGINAL_SERVER_PID=$!

echo "==== Starting Summary Model SGLang Server (Port 6002)... ===="
CUDA_VISIBLE_DEVICES=4,5,6,7 python -m sglang.launch_server \
    --model-path $SUMMARY_MODEL_PATH --host 0.0.0.0 --tp 4 --port 6002 --attention-backend flashinfer &
